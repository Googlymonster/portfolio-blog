...
export async function fetchPosts(): Promise<PostMeta[]> {
  ...
  return (data.items || [])
    ...
    // Filter out any posts that lack a slug or title.  Contentful may
    // return draft entries or incomplete items that would break
    // Next.js serialization if they contain undefined values.  We only
    // include posts with both slug and title defined.
